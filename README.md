...
**Live demo:** https://ai-resume-agent-cnbswchzdtr2lvucructeb.streamlit.app

## What it does

Paste a resume and a job description. The app sends both to an LLM (OpenRouter,
OpenAI, or Anthropic) with a structured prompt and gets back:
...
Python · Streamlit (UI) · OpenRouter / OpenAI / Anthropic API (LLM call) ·
JSON-structured prompting for reliable parsing · deployed on Streamlit
Community Cloud.
...
2. Add `LLM_API_KEY` under the app's Advanced settings → Secrets, in the form
   `LLM_API_KEY = "sk-..."`.
3. Deploy.
...
- Swap providers/models from the sidebar without touching code. Default
  provider is OpenRouter, which gives access to many models through one key.
